...
describe('optInTelemetry / optOutTelemetry', () => {
  ...
  // Fork: the opt-in path is removed (ACCOUNTLESS_BUILD), so nothing can switch
  // telemetry back on — no flag is written and PostHog is never reached. The
  // opt-out case below is unchanged and still holds.
  it('refuses opt-in: nothing is recorded and posthog is never called', () => {
    optInTelemetry();
